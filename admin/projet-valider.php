...
<?php foreach($projet as $categorieitem):
 ...
 <td>
    ...
    <a href="projet-vd-demarche.php?id=<?= $categorieitem['id']; ?> " class="btn btn-success btn-sm">View</a>
     <a href="projet-delete.php?id=<?= $categorieitem['id']; ?>"
...
<script>
    // Toggle between "Refuser" and "Accepter" and change color to green
    $(document).ready(function(){
        $('.accept-reject-toggle').click(function(){
            var $badge = $(this);
            var status = $badge.text().trim();
            if(status === "Refuser"){
                $badge.removeClass("bg-danger").addClass("bg-success").text("Accepter");
            } else {
                $badge.removeClass("bg-success").addClass("bg-danger").text("Refuser");
            }
        });
    });
</script>