...
$(document).ready(function(){
	...
	function add_dynamic_input_field(count)
	{
		...
		output += '<td><input type="text" name="fichier_administratif[]" placeholder="Add fichier_administratif" class="form-control name_list" /></td>';
	...
	$(document).on('click', '.edit', function(){
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				$('#action').val('edit');
				$('.modal-title').text("Modifier les détails");
				$('#submit').val("Edit");
				$('#hidden_id').val(id);
				$('#dynamic_field_modal').modal('show');
			}
